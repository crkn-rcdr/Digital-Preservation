...
sub walk {
    ...
    my $res = $self->repoanalysis->get(
        "/"
          ...
          . "/_design/ra/_view/walkq?reduce=false",
        {},
...
sub load_manifest {
    ...
    # Array of data files
    my @datafiles = grep { /\/data\/files\// } ( split /\n/, $r->content );

    # No longer need manifest.
    undef $r;

    my @revdatafiles =
      grep { /\s+data\/revisions\/([^\/]+)\/data\/files\// } @datafiles;
    my @sipdatafiles = grep { /\s+data\/sip\/data\/files\// } @datafiles;
    ...
    foreach my $line (@datafiles) {
        ...
        if ( $md5 ne $self->aipdata->{$file}->{'hash'} ) {
            die "$file $md5 didn't match Swift hash "
              . $self->aipdata->{$file}->{'hash'} . "\n";
        }
        ...
        if ( substr( $file, 0, 20 ) eq 'data/sip/data/files/' ) {
            ...
            if ( defined $sipdfmd5{"$md5:$length"} ) {
                print STDERR ( $self->aip
                      . ": $md5 with length $length is duplicate between $file and "
                      . $sipdfmd5{"$md5:$length"}
                      . "\n" );
            }
            else {
    ...
    if ( $revfiles != 0 ) {
        print $self->aip
          . " SIP:$sipfiles , Revision:$revfiles , Unique:$unique , Duplicate:$duplicate\n";
    }
}
...
sub load_aipdata {
    ...
    my %containeropt = ( "prefix" => $self->aip . "/" );
    my %aipdata;
...
sub process_sipfiles {
    ...
    foreach my $sipfile ( keys %{ $self->sipfiles } ) {
        ...
        if ($container) {
            ...
            $self->set_fmetadata( $fmetadata, 'jhove_name',
                $jhovefile->{name} );
            $self->set_fmetadata( $fmetadata, 'jhove_hash',
                $jhovefile->{hash} );
            $self->set_fmetadata( $fmetadata, 'jhove_last_modified',
                $jhovefile->{last_modified} );
        }
        else {
