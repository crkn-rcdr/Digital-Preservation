...
sub new {
    ...
    if ( ref($args) ne "HASH" ) {
        die
"Argument to CRKN::Repository::walkjhove::Process->new() not a hash\n";
    }
    $self->{args} = $args;
...
sub process {
    ...
    if ( !( $self->cmdargs->{regenerate} ) ) {
        ...
        my $r =
          $self->swift->object_get( $self->swiftrepoanalysis, "$zipfile" );
        if ( $r->code == 200 ) {
            ...
            die "object_get container: '"
              . $self->swiftrepoanalysis
              . "' , object: '/$zipfile'  returned "
              . $r->code . " - "
              . $r->message . "\n";
        }
    }
    ...
    foreach my $sipfile ( keys %{ $self->repodoc->{sipfiles} } ) {
        ...
        if ($zipfile) {
            ...
            my $pathname =
              $fileinfo->{jhove_container} . " / " . $fileinfo->{jhove_name};
            ...
            if ($jhovetxt) {
                $zip->addString( $jhovetxt, $xmlfilename );
                $self->set_fmetadata( $fileinfo, 'jhove_container',
                    'generate' );
                $zipmodified = 1;
            }
        }
    ...
    if ($zipmodified) {
        ...
        close $fh;
        $self->log->info(
            "Stored " . $self->swiftrepoanalysis . " : $zipfile" );
    }
}
...
sub processjhove {
    ...
    if ($jhoverelease) {
        $self->set_fmetadata( $fileinfo, 'jhoverelease', $jhoverelease );
    }
    else {
        $self->delete_fmetadata( $fileinfo, 'jhoverelease' );
    }

    my $size = $xpc->findvalue( "descendant::jhove:size", $jhove );
    if ( $size > 0 ) {
        $self->set_fmetadata( $fileinfo, 'Size', $size );
    }
    else {
        $self->delete_fmetadata( $fileinfo, 'Size' );
    }

    my $md5 =
      $xpc->findvalue( 'descendant::jhove:checksum[@type="MD5"]', $jhove );
    if ($md5) {
        $self->set_fmetadata( $fileinfo, 'MD5', $md5 );
    }
    else {
        $self->delete_fmetadata( $fileinfo, 'MD5' );
    }
    ...
    if ($mimetype) {
        ...
        if ( index( $mimetype, "image/" ) == 0 ) {
            ...
            if ( scalar(@mix) > 0 ) {
                ...
                $self->delete_fmetadata( $fileinfo, 'Width' );
                $self->delete_fmetadata( $fileinfo, 'Height' );
            }
        }
    }
    else {
        $self->delete_fmetadata( $fileinfo, 'mimetype' );
        $self->delete_fmetadata( $fileinfo, 'Width' );
        $self->delete_fmetadata( $fileinfo, 'Height' );
    }
    ...
    foreach my $errormsg (
        $xpc->findnodes(
            'descendant::jhove:message[@severity="error"]', $jhove
        )
      )
    {
    ...
    if (@errormsgs) {
        ...
        $self->delete_fmetadata( $fileinfo, 'errormsg' );
    }
}
...
sub load_repoanalysisf {
    ...
    my $res = $self->repoanalysisf->get(
        "/" . $self->repoanalysisf->database . "/_all_docs",
        {
            include_docs => 'true',
            startkey     => '"' . $self->aip . '/"',
...
sub save_repoanalysisf {
    ...
    if ( $self->raf ) {
        ...
        if (@update) {
            my $res = $self->repoanalysisf->post(
                "/" . $self->repoanalysisf->database . "/_bulk_docs",
                { docs         => \@update },
                { deserializer => 'application/json' }
            );
            if ( $res->code != 201 ) {
                die "save_repoanalysisf _bulk_docs returned: "
                  . $res->code . "\n";
            }
        }
    }
...
sub generate_jhove {
    ...
    my @command = (
        "/opt/jhove/jhove",    # TODO: in config file?
