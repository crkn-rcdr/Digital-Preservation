    ...
    if ( $self->args->{md5} ) {
        ...
        while ( my $aipinfo = $self->getnextmd5 ) {
            ...
            if ( $id eq $lastid ) {
                ...
                $lastid = $id;
                print $self->repoanalysis->create_or_update( $id,
                    \%repoanalysis )
                  . "\n";
            }
        }
    ...
    if ( $self->args->{nojhove} ) {
        ...
        my $res = $self->repoanalysisf->get(
            "/"
              ...
              . "/_design/ra/_view/nojhove?reduce=true&group=true&include_docs=false",
            {},
...
#    my $res = $self->repoanalysis->get("/".$self->repoanalysis->{database}."/_design/ra/_view/sipdupinother?reduce=false&limit=1&include_docs=false",{}, {deserializer => 'application/json'});
    my $res = $self->repoanalysis->get(
        "/"
          ...
          . "/_design/ra/_view/dupinother?reduce=false&limit=1&include_docs=false",
        {},
        { deserializer => 'application/json' }
    );
...
sub getnextwalkr {
    ...
    my $res = $self->repoanalysis->get(
        "/"
          . $self->repoanalysis->{database}
          . "/_design/ra/_view/walkr?reduce=false&limit=1&include_docs=false",
        {},
        { deserializer => 'application/json' }
    );
...
sub getnextwalkmetadatar {
    ...
    my $res = $self->repoanalysis->get(
        "/"
          . $self->repoanalysis->{database}
          . "/_design/ra/_view/walkmetadatar?reduce=false&limit=1&include_docs=false",
        {},
        { deserializer => 'application/json' }
    );
