    ...
    until ( $rows == 0 ) {
        my $res = $self->repoanalysis->get(
            "/"
              ...
              . "/_design/filemap/_view/md5sizesip?group=true&group_level=1&limit=$limit&skip=$skip",
            {},
        ...
        if ( $res->code == 200 ) {
            if ( exists $res->data->{rows} ) {
                ...
                $skip = $skip + $rows;
                print "Skip=$skip md5s="
                  . scalar(@md5s)
                  . " First was="
                  . $res->data->{rows}->[0]->{key}[0] . "\n";
            }
            else {
                warn "No {rows}\n";
                $rows = 0;
...
sub process_md5 {
    ...
    my $res = $self->repoanalysis->get(
        "/"
          . $self->repoanalysis->{database}
          . "/_design/filemap/_view/md5sizesip?reduce=false&startkey=\[\"$md5\"\]&endkey=\[\"$md5\",{}\]",
        {},
        { deserializer => 'application/json' }
    );
    if ( $res->code == 200 ) {
        if ( exists $res->data->{rows} ) {
            ...
            foreach my $mss ( @{ $res->data->{rows} } ) {
                if ( !defined $size ) {
                    ...
                    if ( $size != $mss->{key}[1] ) {
                        print "\nFound multiple sizes for $md5"
                          . Dumper( $res->data->{rows} ) . "\n";
                        last;
                    }
                }
            }
