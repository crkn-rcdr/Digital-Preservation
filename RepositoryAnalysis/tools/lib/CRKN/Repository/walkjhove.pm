...
sub walk {
    ...
    my $pool =
      AnyEvent::Fork->new->require("CRKN::Repository::walkjhove::Worker")
...
sub getNextAIP {
    ...
    if ( !defined $self->{nojhove} ) {
        ...
        if ( $self->{nojhovecount} ) {
            $self->log->info( "There are "
                  . $self->{nojhovecount}
                  . " AIPs with missing JHOVE reports" );
        }
    }
    ...
    if ( !defined $self->{walkjhoveq} ) {
        ...
        my $res = $self->repoanalysis->get(
            "/"
              ...
              . "/_design/ra/_view/walkjhoveq?reduce=false",
            {},
            { deserializer => 'application/json' }
        );
        if ( $res->code == 200 ) {
        ...
        if ( $self->{walkjhoveqcount} ) {
            $self->log->info( "There are "
                  . $self->{walkjhoveqcount}
                  . " AIPs needing jhove reports processed" );
        }
    }
